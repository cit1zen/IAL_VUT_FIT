...
**/
    //Alokujeme si premennu
    L=malloc(sizeof(tList));
    //Ak by malloc nefungoval
    if(L==NULL)
    {
        Error();
    }
    ...
    {
        //Nastavime premenne na bezpecne hodnoty
        L->Act=NULL;
        L->First=NULL;
    }
...
***/
    ...
    if(L!=NULL && L->First!=NULL)
    {
        ...
        do
        {
            tElemPtr nasledujuci_prvok=L->First->ptr;
            free(L->First);
...
** volá funkci Error().
**/
    tElemPtr nasledujuci_prvok=L->First;
    //Alokovanie noveho prvku
    L->First=malloc(sizeof(struct tElem));
    if(L->First==NULL)
        Error();

    //Nastavenie prvkov
    L->First->ptr=nasledujuci_prvok;
    L->First->data=val;
...
**/
    ...
    {
        tElemPtr nasledujuci_prvok=L->Act->ptr->ptr;
        free(L->First->ptr);
        L->Act->ptr=nasledujuci_prvok;
    }
 //solved = FALSE;                   /* V pøípadì øe¹ení, sma¾te tento øádek! */
}
...
**/
    if(L->Act!=NULL)
    {
        tElemPtr nasledujuci_prvok=L->Act->ptr;
        //Alokovanie noveho prvku
        L->Act->ptr=malloc(sizeof(struct tElem));
        if(L->Act->ptr==NULL)
            Error();

        //Nastavenie prvkov
        L->Act->ptr->ptr=nasledujuci_prvok;
        L->Act->ptr->data=val;
